...
private:
    ...
    GLuint nbo; // Change int to GLuint
...
WindowsWave::WindowsWave(int width, int height, const std::string& title) {
    ...
    wc.lpszClassName = L"WindowsWaveClass"; // Use wide string literal
    RegisterClass(&wc);

    // Create the window
    hwnd = CreateWindowEx(
        0,
        L"WindowsWaveClass", // Use wide string literal
        std::wstring(title.begin(), title.end()).c_str(), // Convert std::string to std::wstring
        WS_OVERLAPPEDWINDOW,
